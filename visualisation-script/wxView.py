...
from psrpoppysuper import dataobj 
...
class VisualizeFrame(wx.Frame):
    ...
    def create_main_panel(self):
        ...
        self.modelCheckList = wx.CheckListBox(self.panel, -1,
                                            choices=modelList,
                                            style=wx.ALIGN_RIGHT)
        self.modelCheckList.SetCheckedItems(range(len(modelList)))
    ...
    def onXRadioClick(self, event):
        # gets xIndex of selected button
    ...
    def draw_figure(self):
        self.axes.clear()
        for dataObjIndex in self.modelCheckList.GetCheckedItems():
            try:
        ...
        if len(self.modelCheckList.GetCheckedItems())>0:
