...
try:
    ...
    try:
        ...
        import psrpoppysuper.distributions as dists
        import psrpoppysuper.galacticops as go
        # import psrpoppysuper.orbitalparams

        from psrpoppysuper.population import Population
        from psrpoppysuper.pulsar import Pulsar
        from psrpoppysuper.survey import Survey

        from psrpoppysuper.progressbar import ProgressBar
    except ImportError:
...
def generate(ngen,
    ...
    if electronModel not in ['ne2001', 'lmt85','ne2025','ymw16']:
    ...
    try:
        ...
        # snapshot locals() to avoid comprehension/lookup fragility
        _locals_snapshot = dict(locals())
        key_values = [(arg, _locals_snapshot.get(arg)) for arg in arg_names]

        # cast to Any to satisfy type checkers if Population.arguments isn't annotated
        from typing import Any, cast
    ...
    while pop.ndet < ngen:
        ...
        # none of these types.
        # Guard against gpsFrac being None (gpsArgs default is [None, None]).
        # If gpsFrac is None treat it as 0 (i.e. no GPS sources).
        if pop.gpsFrac is None or random.random() > float(pop.gpsFrac):
            p.gpsFlag = 0
        else:
            p.gpsFlag = 1
            p.gpsA = pop.gpsA

        # Guard against brokenFrac being None; treat None as 0 (no broken-SI sources)
        if pop.brokenFrac is None or random.random() > float(pop.brokenFrac):
            p.brokenFlag = 0
        else:
            p.brokenFlag = 1
        ...
        # then calc DM  using fortran/C libs
...
def main():
    ...
    parser.add_argument('-dm', type=str, nargs=1, required=False,
                        ...
                        choices=['ne2001','lmt85','ne2025','ymw16'])
