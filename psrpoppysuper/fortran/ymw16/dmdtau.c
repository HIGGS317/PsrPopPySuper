...
double dmdtau_value(double gl, double gb ,double dordm, double DM_Host, int ndir, int np, int vbs, char *dirname, double *tau_sc_out)
{
  double ne0=0;
  double ne=0;
  double ne1=0;
  ...
  double ne9=0;
  double ne10=0;
  double dist = 0, xx, yy, zz, r, glr, gbr, sl, cl, sb, cb, hh;

  double nstep, dstep, dmstep;
  double dd, dtest, dmpsr = 0, rr;
  double dmm=0;
  double dm=0;
  double DM_MC=0;
  double DM_Gal=0;
  ...
  //The location of Sun relative to GP and Warp
  double z_warp, zz_w, R_warp, theta_warp, theta_max;
  R_warp=8400;//pc
  theta_max=0.0; //In +x direction
    ...
    if(ndir==1){
      if(dmm<=dm){
        ...
        if(i==nk){ 
          ...
          if(tau_sc_out) {
            *tau_sc_out = (np == 0) ? tau_MC_sc : tau_sc;
          }
          return dist;
        }	    
      }
    ...
    if(ndir==2){
      if(dd<=dtest){
        ...
        if(i==nk&&np==-1){
          ...
          double c_light=3e8;
          double H_0=2.1808e-18;
          double n_IG=0.16;
          double z_val, DM_IGM_val, DDM_val;
          double dist_m = dordm*3.086e22;
          z_val = exp(dist_m*H_0/c_light)-1;
          DM_IGM_val = z_val*c_light*n_IG/H_0/3.086e22;
          DDM_val = DM_IGM_val + DM_Gal + DM_MC + DM_Host;
          
          double t_Gal = 0.5*tsc(DM_Gal);
          double t_MC = 0.5*tsc(DM_MC);
          double t_IGM = pow(10,-6.4)*pow(DM_IGM_val, 1.3);
          double t_Host = 0.5*tsc(DM_Host*(1+z_val))*pow((1+z_val),-3.0);
          double t_FRB = MAX(t_Gal, t_MC);
          t_FRB = MAX(t_FRB, t_IGM);
          t_FRB = MAX(t_FRB, t_Host);
          if(tau_sc_out) {
            *tau_sc_out = t_FRB;
          }
          return DDM_val;
